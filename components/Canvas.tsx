...
export default function Canvas({ theme, wsInstance, prediction }: CanvasProps) {
	...
	function getCropRectangle(): { top: number; bottom: number; left: number; right: number } | undefined {
		...
		<div className="w-screen h-screen flex flex-col items-center justify-center gap-5">
			<div className="flex gap-5 items-center">
				...
				<div className="flex flex-col items-center gap-5">
					...
					<button type="button" onClick={clearCanvas} className="border-2 border-secondary rounded-md p-2">
						<Trash2 size={50} />
					</button>
					<button type="button" onClick={skipTheme} className="border-2 border-secondary rounded-md p-2">
						<ArrowRight size={50} />
					</button>
				</div>
			</div>
			<div className="flex gap-2">
				...
				{prediction === "" ? <p>Please draw something to get started!</p> : <p>That is a(n) {prediction}!</p>}
			</div>
		</div>
	);
}
